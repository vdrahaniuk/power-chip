<?php
  ...
  //	Link 'write a new entry...'
    ...
    <a target="_self" href="'.$gb_links['write'].'">&raquo; ' . __('Отправить отзыв',$textdomain) . '</a>
  </div>';
			...
			$output .= '<div'; if (!$notFirst) { $notFirst = true; $output .= ' id="first"'; } $output .= ' class="gb-entry '; if ($entry['entry_authorAdminId'] > 0) { $output .= 'admin-entry'; } $output .= '">';
				$output .= '<div class="author-info">';
					$output .= '<span class="author-name">'.$entry['entry_author_name_html'].'</span>';
						...
						$output .= '' . __(',',$textdomain) . ' <span class="author-origin">' . gwolle_gb_format_value_for_output($entry['entry_author_origin']) . '</span>';
						$output .= '. ' . __(' ',$textdomain) . ' <span class="author-auto">' . gwolle_gb_format_value_for_output($entry['entry_author_email'])  . 
						'</span>';
						$output .= ' ' . __(',',$textdomain) . ' <span class="auto-capacity">' . gwolle_gb_format_value_for_output($entry['entry_author_website'])  . 
						'</span>';
					#}
					# $output .= ' ' . __(' ',$textdomain) . ' ' . date($date_format, $entry['entry_date']) . ':';
				$output .= '</div>';
