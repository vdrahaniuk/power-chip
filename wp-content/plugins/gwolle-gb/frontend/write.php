<?php
  ...
  $output .= '
	<div>
    <a target="_self" href="'.$gb_links['read'].'">&laquo; ' . __('Назад к записям',$textdomain) . '</a>
  </div>';
  ...
  <form id="new_entry" style="text-align:left;" action="'.$gb_links['write'].'" accept-charset="UTF-8" method="POST">
		...
		<div class="label">'.__('Имя',$textdomain).':*</div>
	  ...
	  if ($gwolle_gb_settings['recaptcha-active'] === TRUE) {
		...
		$output .= '
		<div class="label">&nbsp;</div>
		<div class="input"><input type="submit" value="'.__('Сохранить',$textdomain).'" /></div>
		<div class="clearBoth">&nbsp;</div>
	</form>
