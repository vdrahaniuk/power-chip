							...
							if(isset($egrab->name)){
								...
								foreach($myrowres as $myrow){
									...
									if(!in_array('price',$willhide))
										...
										$tbody.='<td headers="'.$eshopletter.'eshopprice '.$eshopletter.'eshopnumrow'.$i.'"'.$thclass.'>'.$myrow->price.''.$currsymbol.'</td>';
									if($eshopoptions['shipping']=='4' && !in_array('weight',$willhide)){
										/* ,'1- weight 2-weight symbol' */
										$tbody.='<td headers="'.$eshopletter.'eshopweight '.$eshopletter.'eshopnumrow'.$i.'">'.sprintf( __('%1$s %2$s','eshop'), $myrow->weight,__('2','eshop')).'</td>';
									}
									$tbody.="</tr>\n";
									$i++;
								}
