...
global $pri_width_height;
$pri_width_height = "14";
// end settings

//THESE TWO VALUES EDITED BY JUSTIN
global $pri_image_url, $pri_image_path;
$pri_image_url = plugins_url(dirname(dirname(plugin_basename(__FILE__)))) . "/img/browsers";
$pri_image_path = dirname(dirname(__FILE__)) . "/img/browsers";

//THIS FUNCTION ADDED BY JUSTIN
function pri_images_string (	$browser_name = '', $browser_code = '', $browser_ver = '',
				...
				$pda_name= '', $pda_code = '', $pda_ver = '', $image = false, $between = 'on' )
{
	...
	$out .= pri_get_image_url($pda_code, $pda_name." ".$pda_ver);
	$out .= pri_get_image_url($os_code, $os_name." ".$os_ver);
	$out .= pri_get_image_url($browser_code, $browser_name." ".$browser_ver);
	return $out;
}
///////FROM HERE ON THE PLUGIN IS UNMODIFIED////
